<template>
  <div class="res-home">
    ...
    <div
      ...
      @click="navigateTo('member-center')"
    >
      ...
      <div class="member-points">{{ userInfo.points }} 积分</div>
    </div>
...
onMounted(async () => {
  ...
  if (cats.length) {
    ...
    displayCategories.value = [
      { name: '招牌菜', icon: '🍳' }, { name: '主食', icon: '🍚' },
      { name: '小吃', icon: '🥟' }, { name: '饮品', icon: '🧋' },
      { name: '甜品', icon: '🍰' }, { name: '套餐', icon: '🍱' },
      { name: '会员专区', icon: '👑' }, { name: '活动专区', icon: '🎉' },
    ];
  }
...
.member-card {
  ...
  .member-info {
    ...
    .member-level { font-size: 11px; opacity: 0.8; }
  }
  .member-points { font-size: 16px; font-weight: 700; }
}
</style>
