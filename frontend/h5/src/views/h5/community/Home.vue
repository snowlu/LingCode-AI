    ...
    <div class="h5-banner">
      <el-carousel height="100px" :interval="4000" indicator-position="outside">
        <el-carousel-item v-for="(b, i) in banners" :key="i">
          <div class="banner-slide" :style="{ background: b.bg }"><h3>{{ b.title }}</h3><p>{{ b.subtitle }}</p></div>
        </el-carousel-item>
      </el-carousel>
    </div>
...
const banners = [
  ...
  { title: '在线报修', subtitle: '一键报修，快速响应', bg: 'linear-gradient(135deg, #23d160, #48d88a)' },
];
const categories = [
  { name: '在线报修', icon: '🔧', route: 'repair-apply' },
  { name: '活动报名', icon: '🎉', route: 'activity-list' },
  { name: '社区公告', icon: '📋', route: 'notice-list' },
  { name: '我的报修', icon: '📝', route: 'my-repairs' },
];
